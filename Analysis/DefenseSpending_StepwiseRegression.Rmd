...
The direction of the UN troops + experts per million in population was surprising, as we had intuited that it may build support for spending. On closer examination, it appears driven by a UN mission to Lebanon, UNIFIL, that ramped up during the same period where spending was dropping accross multiple countries in Europe. There's no clear causal mechanism and no support and literature, so it will be left out. International conflict is actually a challenging variable as it's only true in 6 instances, 5 of which being Turkey. Participation in operations in Afghanistan does not qualify. Given the limited relevance and the risk that it's a dummy variable for Turkey, it will be left out.
...
reg<-10
lmMoreDef[reg]<-list(lm(log(DefSpend_lead) ~ DefSpread_lag1 + log(NGDPPC_eu2014)+
                            log(Population)+Debt_NGDP+EUdebt_NGDP+Dfc_NGDP+DomAt, EuropeMoreDef))
lmEUldr[reg]<-list(lm(log(DefSpend_lead) ~ EUldrSpread_lag1 + log(NGDPPC_eu2014)+
...
reg<-11
lmMoreDef[reg]<-list(lm(log(DefSpend_lead) ~ DefSpread_lag1 + log(NGDPPC_eu2014)+
                            log(Population)+Debt_NGDP+EUdebt_NGDP+Dfc_NGDP+IntAt, EuropeMoreDef))
lmEUldr[reg]<-list(lm(log(DefSpend_lead) ~ EUldrSpread_lag1 + log(NGDPPC_eu2014)+
                            log(Population)+Debt_NGDP+EUdebt_NGDP+Dfc_NGDP+IntAt, EuropeEUleadership))
lmNATOess[reg]<-list(lm(log(DefSpend_lead) ~ NATOessSpread_lag1 + log(NGDPPC_eu2014)+
                            log(Population)+Debt_NGDP+EUdebt_NGDP+Dfc_NGDP+IntAt, EuropeNATOess))
lmEU_UScls[reg]<-list(lm(log(DefSpend_lead) ~ EUcloserUSspread_lag1+ log(NGDPPC_eu2014)+
                            log(Population)+Debt_NGDP+EUdebt_NGDP+Dfc_NGDP+IntAt,
                         EuropeEUcloserUS))
nModel[reg]<-"IntAt"


screenreg(c(lmMoreDef[c(9:11)],
               lmEUldr[c(9:11)]),
               custom.model.names=c(rep(nModel[c(9:11)],2)),
                 stars=c(0.001,0.01,0.05,0.1),
...
GlobalSecurityScatter <- melt(EuropeOmnibus, measure.vars = c("GCivilWarBRDDelt",
                                           "RussiaDefSpendDelt"))

 
    ggplot(data=GlobalSecurityScatter,aes(x=value,
                            y=DefSpendDelt_lead,
                            color=Country))+geom_point()+
        facet_grid(~variable,scale="free_x")+geom_point()+stat_smooth(method = "lm", col = "green") 
...
screenreg(c(lmMoreDef[c(11:13)],
            lmEUldr[c(11:13)],
            lmNATOess[c(11:13)],
            lmEU_UScls[c(11:13)]),
          custom.model.names=c(rep(nModel[c(11:13)],4)),
              stars=c(0.001,0.01,0.05,0.1),
