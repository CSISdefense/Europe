...
#get rid of greece
EuropeOmnibus <- subset(EuropeOmnibus, !Country %in% c("Greece", "Sweden","Russia","USA"))
...
# USleadResults10 <- lm(log(Dspend) ~
#     USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
...
##For IntAt and GDPpc
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults1 <- lm(log(Dspend) ~ USldrSpread + ThrtR + DomAt + CivWr + IntWr + log(Pop) + Dem , EuropeOmnibus)
...
##For CivWr and DomAt
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults2 <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
var.test(USleadResultChosen, Hresults2, ratio = .000001, conf.level = 0.95)
##reject null hypothesis that both of these coefficients are equal to zero

##For CivWr and IntWr
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults3 <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
...
##For CivWr and Dem
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults4 <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + IntWr + log(Pop) + log(GDPpC) , EuropeOmnibus)
...
##For IntWr and Dem
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults5 <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + log(Pop) + log(GDPpC) , EuropeOmnibus)
...
##For GDPpc and CivWr
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults6 <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + IntWr + log(Pop) + Dem , EuropeOmnibus)
var.test(USleadResultChosen, Hresults6, ratio = .000001, conf.level = .95)
##reject the null hypothesis that both of these coefficients are equal to zero

##For Dem and USldrSpread
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults7 <- lm(log(Dspend) ~ ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) , EuropeOmnibus)
var.test(USleadResultChosen, Hresults7, ratio = .000001, conf.level = .95)
## reject the null hypothesis that both of these coefficients are equal to zero

##For CivWr and USldrSpread
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults8 <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
var.test(USleadResultChosen, Hresults8, ratio = .000001, conf.level = .95)
##reject the null hypothesis that both of these coefficients are equal to zero

##for USldrSpread and Dem
USleadResultChosen <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
Hresults9 <- lm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeOmnibus)
...
Bresults1 <- plm(log(Dspend) ~ USldrSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , data=EuropeOmnibus, index=c("Country", "Year"), model="within")
summary(Bresults1)
#plot(Bresults1)
...
Cresults10 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
...
##ThrtR and GDPpc
Cresults10 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
Gresults1 <- lm(log(Dspend) ~ DefSpread + IntAt + DomAt + CivWr + IntWr + log(Pop) + Dem , EuropeDefSpread)
...
##DomAt and CivWr
Cresults10 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
Gresults2 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
var.test(Cresults10, Gresults2, ratio = .000001, conf.level = .95)
##reject the null hypothesis that these two variables are equal to zero

##CivWr and IntWr
Cresults10 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
Gresults3 <-  lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
var.test(Cresults10, Gresults3, ratio = .000001, conf.level = .95)
##reject the null hypothesis that these two variables are equal to zero

##CivWr and Dem
Cresults10 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
Gresults4 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + IntWr + log(Pop) + log(GDPpC) , EuropeDefSpread)
var.test(Cresults10, Gresults4, ratio = .000001, conf.level = .95)
##reject the null hypothesis that these two variables are equal to zero

##IntrWr and Dem
Cresults10 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
Gresults5 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + log(Pop) + log(GDPpC) , EuropeDefSpread)
var.test(Cresults10, Gresults5, ratio = .000001, conf.level = .95)
##reject the null hypothesis that these two variables are equal to zero

##Pop and Dem
Cresults10 <- lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
Gresults6 <-  lm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , EuropeDefSpread)
...
Dresults1 <- plm(log(Dspend) ~ DefSpread + ThrtR + IntAt + DomAt + CivWr + IntWr + log(Pop) + log(GDPpC) + Dem , data=EuropeDefSpread, index=c("Country", "Year"), model="within")
summary(Dresults1)
